    ...
    <?php else: ?>
        ...
        fetch('/wp-admin/admin-ajax.php', {
            ...
            body: new URLSearchParams({
                action: 'clifton_save_search',
                search_name: name,
                query_params: window.location.search.substring(1),
                nonce: '<?php echo wp_create_nonce("clifton_save_search_nonce"); ?>'
            })
        })
        .then(res => res.json())
        .then(data => {
...
document.addEventListener('DOMContentLoaded', function() {
    const bookmarks = <?php echo json_encode(get_user_meta(get_current_user_id(), '_clifton_reading_list', true) ?: array()); ?>;
