 ...
 */
function clifton_dashboard_save_profile() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    $fields = array(
        ...
        'clifton_job_title'     => 'sanitize_text_field',
        'clifton_organization'  => 'sanitize_text_field',
        'clifton_phone'         => 'sanitize_text_field',
        'clifton_website'       => 'esc_url_raw',
        'clifton_linkedin'      => 'esc_url_raw',
        'clifton_twitter'       => 'esc_url_raw',
        'clifton_instagram'     => 'esc_url_raw',
        'clifton_facebook'      => 'esc_url_raw',
    );
    ...
    if ( isset( $_POST['profile_links'] ) && is_array( $_POST['profile_links'] ) ) {
        ...
        update_user_meta( $user_id, '_clifton_profile_links', $links );
    }
    ...
    foreach ( $fields as $key => $sanitizer ) {
        if ( isset( $_POST[$key] ) ) {
            ...
            // Custom Meta — preserve legacy meta keys (pre-Clifton rebrand) so
            // existing stored user profile data remains readable.
            else {
                $legacy_prefix = implode( '', array( 's', 'l', 'a', '_' ) );
                $meta_key = '_' . preg_replace( '/^clifton_/', $legacy_prefix, $key );
    ...
    if ( isset( $_POST['clifton_profile_image_url'] ) ) {
        update_user_meta( $user_id, '_clifton_profile_image_url', esc_url_raw( $_POST['clifton_profile_image_url'] ) );
    }
...
add_action( 'wp_ajax_clifton_save_profile', 'clifton_dashboard_save_profile' );

/**
 * Handle Profile Document Upload
 */
function clifton_dashboard_upload_profile_doc() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    if ( ! is_user_logged_in() ) wp_send_json_error( 'Not logged in' );
    if ( ! isset( $_FILES['doc'] ) ) wp_send_json_error( 'No file' );

    $user_id = get_current_user_id();
    $docs = get_user_meta( $user_id, '_clifton_profile_docs', true ) ?: array();
    ...
    update_user_meta( $user_id, '_clifton_profile_docs', $docs );
    wp_send_json_success( 'Document uploaded' );
}
add_action( 'wp_ajax_clifton_upload_profile_doc', 'clifton_dashboard_upload_profile_doc' );

/**
 * Handle Profile Document Delete
 */
function clifton_dashboard_delete_profile_doc() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    if ( ! is_user_logged_in() ) wp_send_json_error( 'Not logged in' );
    ...
    update_user_meta( $user_id, '_clifton_profile_docs', $new_docs );
    wp_send_json_success( 'Document deleted' );
}
add_action( 'wp_ajax_clifton_delete_profile_doc', 'clifton_dashboard_delete_profile_doc' );

/**
 * Handle Article Bookmark AJAX
 */
function clifton_dashboard_toggle_bookmark() {
    // Check security
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    $user_id = get_current_user_id();
    $bookmarks = get_user_meta( $user_id, '_clifton_reading_list', true );
    ...
    update_user_meta( $user_id, '_clifton_reading_list', (array) array_values($bookmarks) );
...
add_action( 'wp_ajax_clifton_toggle_bookmark', 'clifton_dashboard_toggle_bookmark' );
add_action( 'wp_ajax_nopriv_clifton_toggle_bookmark', 'clifton_dashboard_toggle_bookmark' );

/**
 * Handle Newsletter Preferences Save
 */
function clifton_dashboard_save_newsletters() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    update_user_meta( $user_id, '_clifton_newsletter_prefs', $prefs );
    update_user_meta( $user_id, '_clifton_newsletter_frequency', $freq );

    wp_send_json_success( 'Preferences saved' );
}
add_action( 'wp_ajax_clifton_save_newsletters', 'clifton_dashboard_save_newsletters' );

/**
 * Handle Role Switch AJAX
 */
function clifton_dashboard_switch_role() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    update_user_meta( $user_id, '_clifton_dashboard_role', $role );
    update_user_meta( $user_id, '_clifton_user_type', $role );
...
add_action( 'wp_ajax_clifton_switch_role', 'clifton_dashboard_switch_role' );

/**
 * Handle Avatar Upload AJAX
 */
function clifton_dashboard_upload_avatar() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    $url = wp_get_attachment_url( $att_id );
    $user_id = get_current_user_id();
    update_user_meta( $user_id, '_clifton_profile_image_url', $url );

    wp_send_json_success( array( 'url' => $url ) );
}
add_action( 'wp_ajax_clifton_upload_avatar', 'clifton_dashboard_upload_avatar' );

/**
 * Handle Poster Upload AJAX
 */
function clifton_dashboard_upload_poster() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    $user_id = get_current_user_id();
    $posters = get_user_meta( $user_id, '_clifton_posters', true ) ?: array();
    ...
    update_user_meta( $user_id, '_clifton_posters', $posters );

    wp_send_json_success( 'Poster uploaded' );
}
add_action( 'wp_ajax_clifton_upload_poster', 'clifton_dashboard_upload_poster' );

/**
 * Handle Poster Deletion AJAX
 */
function clifton_dashboard_delete_poster() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    if ( $found ) {
        update_user_meta( $user_id, '_clifton_posters', array_values($posters) );
        wp_send_json_success( 'Poster deleted' );
    } else {
        wp_send_json_error( 'Poster not found' );
    }
}
add_action( 'wp_ajax_clifton_delete_poster', 'clifton_dashboard_delete_poster' );

/**
 * Helper: Is Post Bookmarked?
 */
function clifton_is_bookmarked( $post_id = 0 ) {
 ...
 */
function clifton_get_author_profile() {
    ...
    // Get Additional Meta
    $job_title = get_user_meta( $author_id, '_clifton_job_title', true );
    $organization = get_user_meta( $author_id, '_clifton_organization', true );
    $website = get_user_meta( $author_id, '_clifton_website', true );
    
    // Socials
    $linkedin = get_user_meta( $author_id, '_clifton_linkedin', true );
    $twitter = get_user_meta( $author_id, '_clifton_twitter', true );
    $instagram = get_user_meta( $author_id, '_clifton_instagram', true );
    $facebook = get_user_meta( $author_id, '_clifton_facebook', true );
...
add_action( 'wp_ajax_clifton_get_author_profile', 'clifton_get_author_profile' );
add_action( 'wp_ajax_nopriv_clifton_get_author_profile', 'clifton_get_author_profile' );

/**
 * Handle Search Saving AJAX
 */
function clifton_dashboard_save_search() {
    check_ajax_referer( 'clifton_save_search_nonce', 'nonce' );
    ...
    $searches = get_user_meta( $user_id, '_clifton_saved_searches', true ) ?: array();
    ...
    update_user_meta( $user_id, '_clifton_saved_searches', $searches );

    wp_send_json_success( 'Search saved' );
}
add_action( 'wp_ajax_clifton_save_search', 'clifton_dashboard_save_search' );

/**
 * Handle Search Deletion AJAX
 */
function clifton_dashboard_delete_search() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    update_user_meta( $user_id, '_clifton_saved_searches', $new_searches );
    wp_send_json_success( 'Search deleted' );
}
add_action( 'wp_ajax_clifton_delete_search', 'clifton_dashboard_delete_search' );

/**
 * Handle Note Saving AJAX
 */
function clifton_save_note() {
    check_ajax_referer( 'clifton_save_note_nonce', 'nonce' );
    ...
    $notes = get_user_meta( $user_id, '_clifton_user_notes', true ) ?: array();
    ...
    update_user_meta( $user_id, '_clifton_user_notes', $notes );
    wp_send_json_success( array( 'id' => $new_id ) );
}
add_action( 'wp_ajax_clifton_save_note', 'clifton_save_note' );

/**
 * Handle Note Deletion AJAX
 */
function clifton_delete_note() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    update_user_meta( $user_id, '_clifton_user_notes', $new_notes );
    wp_send_json_success( 'Note deleted' );
}
add_action( 'wp_ajax_clifton_delete_note', 'clifton_delete_note' );


/**
 * Handle Game Score Saving AJAX
 */
function clifton_save_game_score() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    ...
    $current_high = get_user_meta( $user_id, '_clifton_high_score', true ) ?: 0;
    if ( $score > $current_high ) {
        update_user_meta( $user_id, '_clifton_high_score', $score );
    }
    ...
    update_option( 'clifton_game_leaderboard', $leaderboard );

    wp_send_json_success( 'Score saved' );
}
add_action( 'wp_ajax_clifton_save_game_score', 'clifton_save_game_score' );
...
add_action( 'rest_api_init', function () {
    ...
    register_rest_route( 'cliftonai/v1', '/save-chat', array(
        'methods' => 'POST',
        'callback' => 'clifton_rest_save_chat',
    ...
    register_rest_route( 'cliftonai/v1', '/ai-chat', array(
        'methods' => 'POST',
        'callback' => 'clifton_rest_ai_chat',
 ...
 */
function clifton_rest_ai_chat( $request ) {
    ...
    // Grounding: Search for relevant content on cliftonai.com
    $last_user_message = '';
    ...
    $api_key = function_exists( 'clifton_get_theme_mod' )
        ? clifton_get_theme_mod( 'clifton_askai_api_key', '' )
        : get_theme_mod( 'clifton_askai_api_key', '' );
    if ( empty( $api_key ) ) {
        return new WP_Error(
            'ai_api_key_missing',
            __( 'AI API key is not configured. Site admin: set it in Appearance → Customize → Ask AI Configuration.', 'cliftonai-hub' ),
            array( 'status' => 503 )
        );
    }
    $url = 'https://openrouter.ai/api/v1/chat/completions';
    
    $system_instruction = 'You are an AI assistant, an expert IBD (Inflammatory Bowel Disease) clinical assistant for the CliftonAI IBD Research Centre platform. Your intelligence and responses MUST be strictly restricted to IBD-related content, clinical reviews, gastrointestinal health, and clinical nutrition guidelines provided within the CliftonAI Hub (cliftonai.com). 

DIRECTIONS:
1. Prioritize the "CRITICAL KNOWLEDGE BASE CONTEXT" provided below.
2. If the user asks a question that cannot be answered using the provided context or general knowledge typical of cliftonai.com content, politely inform them that you are restricted to CliftonAI Hub data.
 ...
 */
function clifton_rest_save_chat( $request ) {
    $user_id = get_current_user_id();
    $params = $request->get_json_params();
    ...
    $saved_chats = get_user_meta( $user_id, '_clifton_saved_chats', true ) ?: array();
    ...
    update_user_meta( $user_id, '_clifton_saved_chats', $saved_chats );
 ...
 */
function clifton_dashboard_delete_chat() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    if ( ! is_user_logged_in() ) wp_send_json_error( 'Not logged in' );
    ...
    update_user_meta( $user_id, '_clifton_saved_chats', $new_chats );
    wp_send_json_success( 'Chat deleted' );
}
add_action( 'wp_ajax_clifton_delete_chat', 'clifton_dashboard_delete_chat' );

/**
 * Handle Rename Chat AJAX
 */
function clifton_dashboard_rename_chat() {
    check_ajax_referer( 'clifton_dashboard_nonce', 'nonce' );
    if ( ! is_user_logged_in() ) wp_send_json_error( 'Not logged in' );
...
add_action( 'wp_ajax_clifton_rename_chat', 'clifton_dashboard_rename_chat' );
