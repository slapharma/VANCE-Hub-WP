...
if ( ! clifton_get_theme_mod( 'clifton_show_inner_nav', true ) ) {
    return;
}
...
$total_items = clifton_get_theme_mod( 'clifton_inner_nav_total_items', 8 );
$col_count = clifton_get_theme_mod( 'clifton_inner_nav_cards_per_row', 8 );
        ...
        <div class="inner-cat-nav" style="pointer-events: auto;">
            ...
            ?>
                <a href="<?php echo get_category_link( $cat->term_id ); ?>" class="cat-mini-card <?php echo $is_active ? 'active' : ''; ?>" style="<?php echo $card_style; ?>" title="<?php echo esc_attr( $cat->name ); ?>">
                    <?php 
                    $cat_icon = $icon ?: clifton_get_category_icon_url($cat->name);
                    ?>
