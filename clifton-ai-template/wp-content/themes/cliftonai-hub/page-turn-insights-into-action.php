<?php
/**
 * Template Name: Turn Insights into Action
 *
 * Auto-bound to any Page with slug `turn-insights-into-action` via WP's
 * template hierarchy; also selectable from the Page Attributes template
 * dropdown (Template Name above).
 *
 * All user-facing copy is read through clifton_get_theme_mod() with sensible
 ...
 * `clifton_evidence_*` naming used in this file (mirror the pattern in
...
<main id="main-content">
    ...
    <!-- HERO -->
    <?php
    $hero_bg    = clifton_get_theme_mod( 'clifton_evidence_hero_bg', get_template_directory_uri() . '/assets/img/hcp_hero.png' );
    $hero_tag   = clifton_get_theme_mod( 'clifton_evidence_hero_tag', 'Evidence to Practice' );
    $hero_title = clifton_get_theme_mod( 'clifton_evidence_hero_title', 'Turn <span class="highlight">Evidence</span> into Action' );
    $hero_desc  = clifton_get_theme_mod( 'clifton_evidence_hero_desc', 'Rigorous clinical research only matters when it reaches the patient. CliftonAI translates peer-reviewed science and real-world data into practical protocols that clinicians and patients can act on.' );
    $hero_btn1  = clifton_get_theme_mod( 'clifton_evidence_hero_btn1_text', 'Explore the Evidence Library' );
    $hero_btn1_link = clifton_get_theme_mod( 'clifton_evidence_hero_btn1_link', '#pillars' );
    $hero_btn2  = clifton_get_theme_mod( 'clifton_evidence_hero_btn2_text', 'Request a Clinical Consultation' );
    $hero_btn2_link = clifton_get_theme_mod( 'clifton_evidence_hero_btn2_link', '/contact-us/' );
    ?>
    ...
    <!-- EVIDENCE PILLARS -->
    <?php
    $pillars_tag   = clifton_get_theme_mod( 'clifton_evidence_pillars_tag',   'Our Evidence Standards' );
    $pillars_title = clifton_get_theme_mod( 'clifton_evidence_pillars_title', 'Four Sources. One Standard.' );
    $pillars_desc  = clifton_get_theme_mod( 'clifton_evidence_pillars_desc',  'Every recommendation we publish is anchored in at least one of these evidence streams and graded against internationally-recognised quality criteria.' );
    ...
    <section id="pillars" class="section-padding" style="background: var(--accent-color);">
        <div class="container">
            ...
            <div class="grid-2 resource-grid">
                <?php for ( $i = 1; $i <= 4; $i++ ) :
                    $card_title = clifton_get_theme_mod( "clifton_evidence_pillar{$i}_title", $pillar_defaults[ $i ][0] );
                    $card_desc  = clifton_get_theme_mod( "clifton_evidence_pillar{$i}_desc",  $pillar_defaults[ $i ][1] );
                ?>
    ...
    <?php
    $proc_title = clifton_get_theme_mod( 'clifton_evidence_proc_title', 'From Insight to Practice' );
    $proc_desc  = clifton_get_theme_mod( 'clifton_evidence_proc_desc',  'The journey every piece of evidence takes before it reaches a clinician protocol or a patient-facing recommendation.' );
    ...
    $proc_defaults = array(
        ...
        3 => array( 'Apply',      'Protocols feed the CliftonAI dashboard, the Ask AI assistant, and downloadable handouts — so evidence becomes a concrete decision at the point of care.' ),
    );
    ?>
    <section class="section-padding" style="background: white;">
        <div class="container">
            ...
            <div class="grid-3" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px;">
                <?php for ( $i = 1; $i <= 3; $i++ ) :
                    $step_title = clifton_get_theme_mod( "clifton_evidence_proc{$i}_title", $proc_defaults[ $i ][0] );
                    $step_desc  = clifton_get_theme_mod( "clifton_evidence_proc{$i}_desc",  $proc_defaults[ $i ][1] );
                ?>
    ...
    <?php
    $feat_title = clifton_get_theme_mod( 'clifton_evidence_feat_title', 'Latest Evidence in Focus' );
    $feat_desc  = clifton_get_theme_mod( 'clifton_evidence_feat_desc',  'Recent reviews, trial readouts, and protocol updates published by the CliftonAI editorial team.' );
    $feat_cat   = (int) clifton_get_theme_mod( 'clifton_evidence_feat_category', 0 );
    $feat_count = (int) clifton_get_theme_mod( 'clifton_evidence_feat_count', 3 );
    ...
    <!-- CTA -->
    <?php
    $cta_title = clifton_get_theme_mod( 'clifton_evidence_cta_title', 'Put Evidence to Work for Your Patients' );
    $cta_desc  = clifton_get_theme_mod( 'clifton_evidence_cta_desc',  'Free registration unlocks the full protocol library, the Ask AI clinical assistant, and printable patient handouts branded to your practice.' );
    $cta_btn1  = clifton_get_theme_mod( 'clifton_evidence_cta_btn1_text', 'Register Free' );
    $cta_btn1_link = clifton_get_theme_mod( 'clifton_evidence_cta_btn1_link', '/register/' );
    $cta_btn2  = clifton_get_theme_mod( 'clifton_evidence_cta_btn2_text', 'Talk to Our Team' );
    $cta_btn2_link = clifton_get_theme_mod( 'clifton_evidence_cta_btn2_link', '/contact-us/' );
    ?>
