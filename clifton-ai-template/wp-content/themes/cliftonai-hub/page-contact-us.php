...
if ( isset( $_POST['clifton_contact_submit'] ) && wp_verify_nonce( $_POST['clifton_contact_nonce'], 'clifton_contact_form' ) ) {
    ...
    if ( empty( $name ) || empty( $email ) || empty( $message ) ) {
        ...
        $subject = $subject ? "Contact: $subject" : "New Contact Form Submission – CliftonAI";
        $body    = "Name: $name\nEmail: $email\n\n$message";
...
<main id="main-content">
    ...
    function clifton_get_style_contact( $prefix, $default_bg = '' ) {
    ...
    <!-- ══ HERO SECTION ══════════════════════════════════════════════════ -->
    <?php
    $hero_img      = clifton_get_theme_mod( 'clifton_contact_hero_img',   get_template_directory_uri() . '/assets/img/hcp_hero.png' );
    $hero_bg_color = clifton_get_theme_mod( 'clifton_contact_hero_bg_color' );
    $hero_tag      = clifton_get_theme_mod( 'clifton_contact_hero_tag',   'Get in Touch' );
    $hero_title    = clifton_get_theme_mod( 'clifton_contact_hero_title', 'We\'d Love to <span class="highlight">Hear From You</span>' );
    $hero_desc     = clifton_get_theme_mod( 'clifton_contact_hero_desc',  'Whether you\'re a patient, healthcare professional, researcher, or media contact — our team is here to help. Reach out and we\'ll respond within one business day.' );

    $hero_styles   = clifton_get_style_contact( 'clifton_contact_hero' );
    ...
    ?>
    <section class="clifton-contact-hero" style="padding: 95px 0 140px; display: flex; align-items: flex-start; <?php echo $hero_bg_style; ?> position: relative; overflow: hidden;">
    ...
    <?php
    $intro_title  = clifton_get_theme_mod( 'clifton_contact_intro_title', 'How Can We Help?' );
    $intro_text   = clifton_get_theme_mod( 'clifton_contact_intro_text',  'CliftonAI is committed to providing exceptional support to every member of our community. Use the form to send us a message, or reach us directly through any of the channels below.' );
    $detail_email = clifton_get_theme_mod( 'clifton_contact_email',   'info@cliftonai.com' );
    $detail_phone = clifton_get_theme_mod( 'clifton_contact_phone',   '+44 (0)1628 526 005' );
    $detail_addr  = clifton_get_theme_mod( 'clifton_contact_address', 'CliftonAI Solutions Ltd, 4 Renaissance Way, Wooburn Green, HP10 0DF, United Kingdom' );
    $detail_hours = clifton_get_theme_mod( 'clifton_contact_hours',   'Monday – Friday, 9:00 am – 5:00 pm GMT' );

    $social_linkedin  = clifton_get_theme_mod( 'clifton_social_linkedin' );
    $social_facebook  = clifton_get_theme_mod( 'clifton_social_facebook' );
    $social_twitter   = clifton_get_theme_mod( 'clifton_social_twitter' );
    $social_instagram = clifton_get_theme_mod( 'clifton_social_instagram' );
    ?>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1.4fr; gap: 60px; align-items: start;">
                ...
                <div style="background: white; border-radius: 20px; padding: 48px 44px; box-shadow: 0 4px 32px rgba(10,25,41,.1);">
                    ...
                    <form method="post" action="<?php echo esc_url( get_permalink() ); ?>#contact-form" id="contact-form" novalidate>
                        <?php wp_nonce_field( 'clifton_contact_form', 'clifton_contact_nonce' ); ?>
                        ...
                        <div style="margin-bottom: 20px;">
                            ...
                            <select id="contact_subject" name="contact_subject"
                                ...
                                <option value="Enterprise Partner" <?php selected( ( $_POST['contact_subject'] ?? '' ), 'Enterprise Partner' ); ?>>Enterprise Partner</option>
                        ...
                        <button type="submit" name="clifton_contact_submit" value="1" class="btn btn-primary"
