...
function vance_kb_register() {
	register_post_type(
		...
		array(
			'labels'          => array(
				'name'               => __( 'VANCE-Ai KB', 'vance-health-hub' ),
				'singular_name'      => __( 'KB Entry', 'vance-health-hub' ),
				'add_new'            => __( 'Add Entry', 'vance-health-hub' ),
				'add_new_item'       => __( 'Add Knowledge Base Entry', 'vance-health-hub' ),
				'edit_item'          => __( 'Edit Knowledge Base Entry', 'vance-health-hub' ),
				'search_items'       => __( 'Search Knowledge Base', 'vance-health-hub' ),
				'not_found'          => __( 'No knowledge base entries yet.', 'vance-health-hub' ),
				'menu_name'          => __( 'VANCE-Ai KB', 'vance-health-hub' ),
			),
	...
	register_taxonomy(
		...
		array(
			'labels'            => array(
				'name'          => __( 'KB Categories', 'vance-health-hub' ),
				'singular_name' => __( 'KB Category', 'vance-health-hub' ),
...
function vance_kb_seed_terms() {
	...
	$terms = array(
		'Glossary'          => __( 'Definitions of medical and nutritional terms, including acronyms.', 'vance-health-hub' ),
		'About the Hub'     => __( 'What Vance Medical Hub is, what it publishes, who it is for.', 'vance-health-hub' ),
		'Vance Medical'     => __( 'Vance Medical Foods Ltd: company, products, contact.', 'vance-health-hub' ),
		'SLA Pharma'        => __( 'SLA Pharma: the parent entity.', 'vance-health-hub' ),
		'Policies'          => __( 'Editorial, privacy and clinical governance positions.', 'vance-health-hub' ),
		'Uncategorised'     => __( 'Anything not yet filed.', 'vance-health-hub' ),
	);
...
function vance_kb_add_meta_boxes() {
	add_meta_box(
		'vance_kb_settings',
		__( 'Knowledge Base Settings', 'vance-health-hub' ),
	...
	add_meta_box(
		'vance_kb_help',
		__( 'How VANCE-Ai uses this entry', 'vance-health-hub' ),
...
function vance_kb_render_meta_box( $post ) {
	...
	?>
	<p>
		<label for="vance_kb_source_url"><strong><?php esc_html_e( 'Source URL (optional)', 'vance-health-hub' ); ?></strong></label><br>
		...
		<span class="description"><?php esc_html_e( 'Where this came from. Shown to the assistant for provenance; it is never offered to readers as a "Read more" link.', 'vance-health-hub' ); ?></span>
	</p>
	...
	<p>
		...
		<label>
			...
			<strong><?php esc_html_e( 'Always include', 'vance-health-hub' ); ?></strong>
		</label><br>
		<span class="description"><?php esc_html_e( 'Inject this entry into every conversation, whatever the question. Use for core facts (who we are, what the hub is). Keep these few and short, as they are sent on every request.', 'vance-health-hub' ); ?></span>
	</p>

	<p>
		<label for="vance_kb_pdf_id"><strong><?php esc_html_e( 'Attached PDF (optional)', 'vance-health-hub' ); ?></strong></label><br>
		<input type="hidden" id="vance_kb_pdf_id" name="vance_kb_pdf_id" value="<?php echo esc_attr( $pdf_id ); ?>">
		<span id="vance-kb-pdf-name"><?php echo $pdf_url ? esc_html( basename( $pdf_url ) ) : esc_html__( 'None selected', 'vance-health-hub' ); ?></span><br>
		<button type="button" class="button" id="vance-kb-pdf-select"><?php esc_html_e( 'Choose PDF', 'vance-health-hub' ); ?></button>
		<button type="button" class="button" id="vance-kb-pdf-clear"><?php esc_html_e( 'Remove', 'vance-health-hub' ); ?></button>
	</p>

	<?php if ( $pdf_id ) : ?>
		<p>
			<button type="button" class="button button-secondary" id="vance-kb-pdf-extract"><?php esc_html_e( 'Pull text from PDF', 'vance-health-hub' ); ?></button>
			<span class="description"><?php esc_html_e( 'Appends the PDF text to the entry body below, where you can tidy it. Best-effort: if extraction fails, copy and paste the text in yourself. The body is what the assistant reads.', 'vance-health-hub' ); ?></span>
...
function vance_kb_render_help_box() {
	...
	<p style="margin-top:0;">
		<?php esc_html_e( 'The body text below is exactly what VANCE-Ai reads. Write it as plain, factual prose. The assistant quotes and paraphrases from it, so anything vague or out of date will surface in answers.', 'vance-health-hub' ); ?>
	</p>
	<ul style="list-style:disc;margin-left:20px;">
		<li><?php esc_html_e( 'One topic per entry. A glossary term, a company fact, a policy, not a whole document dumped in.', 'vance-health-hub' ); ?></li>
		<li><?php esc_html_e( 'Put the words a reader would actually type into the title and the first line, so keyword matching finds it.', 'vance-health-hub' ); ?></li>
		<li><?php esc_html_e( 'Entries have no public page. The assistant will not offer them as "Read more" links, so put anything you want linked into a published article instead.', 'vance-health-hub' ); ?></li>
		<li><?php esc_html_e( 'Draft entries are ignored. Only published entries reach the assistant.', 'vance-health-hub' ); ?></li>
	</ul>
	<?php
}
...
function vance_kb_ajax_extract_pdf() {
	...
	if ( ! $post_id || ! current_user_can( 'edit_post', $post_id ) ) {
		wp_send_json_error( array( 'message' => __( 'Permission denied.', 'vance-health-hub' ) ) );
	}

	$pdf_id = (int) get_post_meta( $post_id, '_vance_kb_pdf_id', true );
	$path   = $pdf_id ? get_attached_file( $pdf_id ) : '';
	if ( ! $path ) {
		wp_send_json_error( array( 'message' => __( 'No PDF is attached to this entry. Save the entry after choosing one.', 'vance-health-hub' ) ) );
	}
	...
	if ( '' === $text || strlen( $text ) < 40 ) {
		wp_send_json_error(
			array(
				'message' => __( 'Could not read text from this PDF. It is most likely a scan with no text layer. Open it, copy the text, and paste it into the entry body.', 'vance-health-hub' ),
			)
		);
	}
	...
	wp_send_json_success(
		array(
			...
			'message' => sprintf(
				...
				__( 'Pulled %d words in. Please read it through and tidy it before publishing.', 'vance-health-hub' ),
...
function vance_kb_columns( $columns ) {
	...
	foreach ( $columns as $key => $label ) {
		$new[ $key ] = $label;
		if ( 'title' === $key ) {
			$new['vance_kb_always'] = __( 'Always included', 'vance-health-hub' );
		}
	}
	return $new;
}
...
function vance_kb_column_content( $column, $post_id ) {
	...
	echo get_post_meta( $post_id, '_vance_kb_always', true )
		? '<span style="color:#008080;font-weight:600;">' . esc_html__( 'Yes', 'vance-health-hub' ) . '</span>'
		: '-';
}
