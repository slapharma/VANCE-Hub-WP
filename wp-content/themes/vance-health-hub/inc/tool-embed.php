 ...
 */
function vance_tool_embed_shortcode( $atts ) {
    ...
    return sprintf(
        '<div class="tool-embed-container" style="margin: 40px 0;">
            <iframe 
                src="%s" 
                style="width:100%%; height:%s; border:none; border-radius:var(--radius-surface, 24px); box-shadow: 0 4px 6px rgba(0,0,0,0.1);" 
                loading="lazy"
                title="%s"
...
add_shortcode( 'tool_embed', 'vance_tool_embed_shortcode' );

/**
 * Add custom CSS for tool embeds
 */
function vance_tool_embed_styles() {
    echo '
    <style>
...
add_action( 'wp_head', 'vance_tool_embed_styles' );
