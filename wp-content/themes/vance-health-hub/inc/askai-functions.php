...
function vance_ai_reading_levels() {
	return array(
		'simple'        => array(
			'label'       => __( 'Basic', 'vance-health-hub' ),
		...
		'knowledgeable' => array(
			'label'       => __( 'Advanced', 'vance-health-hub' ),
		...
		'expert'        => array(
			'label'       => __( 'Expert', 'vance-health-hub' ),
...
function vance_ai_autosave_conversation( $user_id, $conversation_id, $transcript ) {
	...
	if ( '' === $auto_title ) {
		$auto_title = __( 'AI conversation', 'vance-health-hub' );
	}
...
function vance_rest_ai_chat_clear( $request ) {
	...
	if ( ! preg_match( '/^[a-z0-9-]{8,40}$/', $conversation_id ) ) {
		return new WP_Error( 'bad_conversation', __( 'Invalid conversation id.', 'vance-health-hub' ), array( 'status' => 400 ) );
	}
...
function vance_rest_ai_chat( $request ) {
	...
	if ( empty( $messages ) ) {
		return new WP_Error( 'no_messages', __( 'No messages provided.', 'vance-health-hub' ), array( 'status' => 400 ) );
	}

	if ( vance_ai_is_rate_limited() ) {
		return new WP_Error(
			'rate_limited',
			__( 'You have sent a lot of questions in a short time. Please wait a few minutes and try again.', 'vance-health-hub' ),
			array( 'status' => 429 )
		);
	}
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error(
			'api_error',
			__( 'Failed to connect to the AI service: ', 'vance-health-hub' ) . $response->get_error_message(),
			array( 'status' => 500 )
		);
	}
	...
	if ( 200 !== (int) $code ) {
		$detail = isset( $decoded['error']['message'] ) ? $decoded['error']['message'] : __( 'Unknown error', 'vance-health-hub' );
		return new WP_Error(
			'api_error',
			__( 'The AI service returned an error: ', 'vance-health-hub' ) . $detail,
			array( 'status' => $code )
		);
	}
	...
	if ( '' === trim( $reply ) ) {
		return new WP_Error( 'api_error', __( 'The AI service returned an empty reply.', 'vance-health-hub' ), array( 'status' => 502 ) );
	}
