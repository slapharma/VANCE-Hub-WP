...
function vance_dashboard_features_customize( $wp_customize ) {
	$wp_customize->add_panel( 'vance_dashboard_panel', array(
		'title'       => __( 'Member Dashboard', 'vance-health-hub' ),
		'priority'    => 33,
		'description' => __( 'Show or hide each tab of the member dashboard, rename it, and move it between the sidebar sections. Hiding a feature closes its endpoints too, but never deletes anything a member has already saved.', 'vance-health-hub' ),
	) );

	// --- Section headings --------------------------------------------------
	$wp_customize->add_section( 'vance_dashboard_headings', array(
		'title'       => __( 'Section headings', 'vance-health-hub' ),
		'panel'       => 'vance_dashboard_panel',
		'priority'    => 5,
		'description' => __( 'The small headings above each group in the sidebar. Worth editing after moving features around, since a group holding new things is often no longer described by its original name. A group with no features left in it is hidden automatically.', 'vance-health-hub' ),
	) );
	...
	foreach ( vance_dashboard_sections() as $section => $default_heading ) {
		...
		$wp_customize->add_control( $setting, array(
			'label'       => sprintf( __( '"%s" heading', 'vance-health-hub' ), $default_heading ),
			'description' => __( 'Leave empty to restore the original.', 'vance-health-hub' ),
	...
	foreach ( vance_dashboard_sections() as $slug => $default_heading ) {
		$section_choices[ $slug ] = ( '' === $default_heading )
			? __( 'Top (no heading)', 'vance-health-hub' )
	...
	foreach ( vance_dashboard_features() as $slug => $feature ) {
		...
		if ( ! empty( $feature['toggleable'] ) ) {
			...
			$wp_customize->add_control( $show, array(
				'label'    => __( 'Show this feature', 'vance-health-hub' ),
				'section'  => $section_id,
				'type'     => 'checkbox',
				'priority' => 10,
			) );
		...
		$wp_customize->add_control( $label, array(
			'label'       => __( 'Menu label', 'vance-health-hub' ),
			'description' => sprintf(
				/* translators: %s: the feature's original name. */
				__( 'Shown in the sidebar, the page heading and the dashboard home card. Leave empty to restore "%s".', 'vance-health-hub' ),
				$feature['label']
			),
			'section'     => $section_id,
			'type'        => 'text',
		...
		$wp_customize->add_control( $place, array(
			'label'       => __( 'Sidebar group', 'vance-health-hub' ),
			'description' => __( 'Features appear within a group in the order they are listed in this panel.', 'vance-health-hub' ),
			'section'     => $section_id,
			'type'        => 'select',
