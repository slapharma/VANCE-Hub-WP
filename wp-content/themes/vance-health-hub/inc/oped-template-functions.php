...
function vance_register_digital_assets_meta_boxes() {
    ...
    foreach ( $post_types as $post_type ) {
        add_meta_box(
            'vance_digital_assets',
            'Digital Assets',
            'vance_digital_assets_meta_box_callback',
            $post_type,
            'normal',
            'high'
        );
        ...
        add_meta_box(
            'vance_article_settings',
            'Article Settings',
            'vance_article_settings_meta_box_callback',
            $post_type,
            'side',
            'high'
        );
...
add_action( 'add_meta_boxes', 'vance_register_digital_assets_meta_boxes' );

/**
 * Digital Assets Meta Box Callback
 */
function vance_digital_assets_meta_box_callback( $post ) {
    wp_nonce_field( 'vance_digital_assets_meta_box', 'vance_digital_assets_meta_box_nonce' );
    ...
    $attached_document = get_post_meta( $post->ID, '_sla_attached_document', true );
    ...
    <div class="oped-meta-container">
        ...
        <div class="oped-meta-section">
            ...
            <textarea name="oped_video_summary" id="oped_video_summary" rows="3" style="width: 100%; font-family: monospace; font-size: 13px; padding: 10px; border-radius: var(--radius-field, 16px); border: 1px solid #d1d5db;"><?php echo esc_textarea( $video_summary ); ?></textarea>
        ...
        <div class="oped-meta-section" style="border: 2px solid #008080; background: #FFF8F5;">
            ...
            <textarea name="oped_quiz_embed" id="oped_quiz_embed" rows="4" style="width: 100%; font-family: monospace; font-size: 13px; padding: 10px; border-radius: var(--radius-field, 16px); border: 1px solid #d1d5db;"><?php echo esc_textarea( $quiz_embed ); ?></textarea>
        </div>
        ...
        <div class="oped-meta-section">
            ...
            <input type="hidden" name="vance_attached_document" id="vance_attached_document" value="<?php echo esc_attr( $attached_document ); ?>">
            <button type="button" class="oped-upload-btn" data-target="vance_attached_document" data-type="application">
                <?php echo $attached_document ? 'Change Document' : 'Upload Document'; ?>
            </button>
            <?php if ( $attached_document ) : ?>
                <span class="oped-remove-media" data-target="vance_attached_document">&times; Remove</span>
            <?php endif; ?>
            <div class="oped-media-preview" id="vance_attached_document_preview">
                <?php 
                if ( $attached_document ) {
                    $doc_url = wp_get_attachment_url( $attached_document );
                    $doc_filename = basename( get_attached_file( $attached_document ) );
                    if ( $doc_url ) {
                        echo '<div style="margin-top: 10px; display: flex; align-items: center; gap: 8px; background: white; padding: 8px; border-radius: var(--radius-surface, 24px); border: 1px solid #eee;">
            ...
            mediaUploader.on('select', function() {
                ...
                if (mediaType === 'audio') {
                    ...
                    previewDiv.html('<img src="' + attachment.url + '" alt="Preview">');
                     ...
                     previewDiv.html('<div style="margin-top: 10px; display: flex; align-items: center; gap: 8px; background: white; padding: 8px; border-radius: var(--radius-surface, 24px); border: 1px solid #eee;"><span class="dashicons dashicons-media-default"></span><a href="' + attachment.url + '" target="_blank">' + attachment.filename + '</a></div>');
                }
 ...
 */
function vance_article_settings_meta_box_callback( $post ) {
 ...
 */
function vance_save_digital_assets_meta_boxes( $post_id ) {
    // Verify nonce
    if ( ! isset( $_POST['vance_digital_assets_meta_box_nonce'] ) ) {
        return;
    }
    
    if ( ! wp_verify_nonce( $_POST['vance_digital_assets_meta_box_nonce'], 'vance_digital_assets_meta_box' ) ) {
        return;
    }
    ...
    $fields = array(
        ...
        'vance_attached_document' => '_sla_attached_document',
    );
    ...
    foreach ( $fields as $field_name => $meta_key ) {
        if ( isset( $_POST[ $field_name ] ) ) {
            ...
            if ( in_array( $field_name, array( 'oped_small_infographic', 'oped_large_infographic', 'oped_audio_summary', 'oped_read_time', 'vance_attached_document' ) ) ) {
                $value = absint( $value );
...
add_action( 'save_post', 'vance_save_digital_assets_meta_boxes' );

/**
 * Add Meta Box Instructions
 */
function vance_single_meta_box_instructions() {
    global $post_type;
...
add_action( 'edit_form_top', 'vance_single_meta_box_instructions' );

/**
 * Customize Featured Image Meta Box Title for Op-Ed / Expert Opinions
 */
function vance_custom_featured_image_title( $content, $post_id, $thumbnail_id ) {
...
add_filter( 'admin_post_thumbnail_html', 'vance_custom_featured_image_title', 10, 3 );

/**
 * Add custom image size for hero
 */
function vance_add_custom_image_sizes() {
    add_image_size( 'vance-hero', 1920, 800, true );
    add_image_size( 'vance-infographic-small', 340, 9999, false );
}
add_action( 'after_setup_theme', 'vance_add_custom_image_sizes' );
