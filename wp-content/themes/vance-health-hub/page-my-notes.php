...
if ( $note_id ) {
    $saved_notes = get_user_meta( $user_id, '_sla_user_notes', true ) ?: array();
...
<head>
    ...
    <title>My Notes - Vance Medical Hub</title>
    <?php wp_head(); ?>
    <style>
        ...
        .toolbar { display: flex; gap: 8px; margin-bottom: 16px; padding: 8px; background: white; border-radius: var(--radius-surface, 24px); border: 1px solid #e2e8f0; }
        .toolbar button { background: none; border: none; cursor: pointer; padding: 6px; border-radius: var(--radius-control, 10px); color: #64748b; }
        .toolbar button:hover { background: #f1f5f9; color: #0f172a; }
        .editor-content { flex: 1; background: white; border-radius: var(--radius-surface, 24px); padding: 32px; border: 1px solid #e2e8f0; outline: none; overflow-y: auto; font-size: 16px; line-height: 1.6; color: #334155; min-height: 400px; }
        .btn-save { background: #008080; color: white; border: none; padding: 10px 20px; border-radius: var(--radius-control, 10px); font-weight: 600; cursor: pointer; transition: background 0.2s; }
        .btn-save:hover { background: #006666; }
        .btn-pdf { background: #fff; color: #64748b; border: 1px solid #e2e8f0; padding: 10px 16px; border-radius: var(--radius-control, 10px); font-weight: 600; cursor: pointer; margin-right: 8px; transition: all 0.2s; display: flex; align-items: center; gap: 6px; }
...
function saveNote() {
    ...
    var data = {
        action: 'vance_save_note',
        id: noteId,
        title: title,
        content: content,
        nonce: '<?php echo wp_create_nonce("vance_save_note_nonce"); ?>'
    };
        ...
        <div style="padding: 40px; font-family: Helvetica, Arial, sans-serif; color: #334155;">
            <div style="border-bottom: 2px solid #008080; padding-bottom: 20px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: flex-end;">
                <div>
                   ...
                   <div style="font-size: 12px; color: #64748b; margin-top: 4px;">Vance Medical • IBD Research Centre</div>
                </div>
            ...
            <div style="margin-top: 60px; font-size: 10px; color: #cbd5e1; border-top: 1px solid #e2e8f0; padding-top: 12px; text-align: center;">
                Generated from Vance Medical Hub Dashboard. Private & Confidential.
            </div>
        </div>
    `;
