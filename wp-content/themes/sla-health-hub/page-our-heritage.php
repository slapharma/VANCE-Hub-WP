...
<main id="main-content">
    ...
    if (!function_exists('vance_get_section_style_heritage')) {
        function vance_get_section_style_heritage($prefix, $default_bg = '') {
    ...
    <!-- HERO SECTION -->
    <?php if (vance_get_theme_mod('vance_heritage_hero_show', true)) : 
        $hero_img    = vance_get_theme_mod('vance_heritage_hero_img', get_template_directory_uri() . '/assets/img/hcp_hero.png');
        $hero_bg_color = vance_get_theme_mod('vance_heritage_hero_bg_color');
        $hero_tag   = vance_get_theme_mod('vance_heritage_hero_tag', 'Our Story');
        $hero_title = vance_get_theme_mod('vance_heritage_hero_title', 'From Pharma to <span class="highlight">Healthcare</span>');
        $hero_sub   = vance_get_theme_mod('vance_heritage_hero_sub', 'A Natural Evolution in Gastrointestinal Care');
    ...
    <!-- ORIGIN SECTION -->
    <?php if (vance_get_theme_mod('vance_heritage_origin_show', true)) : 
        $origin_tag   = vance_get_theme_mod('vance_heritage_origin_tag', 'From Pharma to Healthcare');
        $origin_title = vance_get_theme_mod('vance_heritage_origin_title', 'The Vance Evolution');
        $origin_sub   = vance_get_theme_mod('vance_heritage_origin_sub', 'A Natural Evolution in Gastrointestinal Care');
        $styles = vance_get_section_style_heritage('vance_heritage_origin', '#fff');
    ?>
    <section id="our-story" class="section-padding" style="<?php echo $styles['section']; ?>">
        <div class="container">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:32px;text-align:center;padding-top:40px;border-top:1px solid rgba(0,0,0,0.1);">
                ...
                for($i=1; $i<=3; $i++):
                    $s_num = vance_get_theme_mod("vance_heritage_stat{$i}_num", $stat_defaults[$i][0]);
                    $s_lbl = vance_get_theme_mod("vance_heritage_stat{$i}_label", $stat_defaults[$i][1]);
                ?>
                <div>
    ...
    <!-- MISSION & VALUES -->
    <?php if (vance_get_theme_mod('vance_heritage_mission_show', true)) : 
        $mission_tag = vance_get_theme_mod('vance_heritage_mission_tag', 'Our Mission');
        $mission_title = vance_get_theme_mod('vance_heritage_mission_title', 'Bridging Science & <span class="highlight">Patient Wellbeing</span>');
        $mission_desc = vance_get_theme_mod('vance_heritage_mission_desc', 'At Vance Medical, our mission is to empower patients living with chronic gastrointestinal conditions by making world-class clinical nutrition science accessible, actionable, and personal.');
        $styles = vance_get_section_style_heritage('vance_heritage_mission', '#f8f9fa');
    ?>
    ...
    <!-- EPAVANCE SPOTLIGHT -->
    <?php if (vance_get_theme_mod('vance_heritage_product_show', true)) : 
        $prod_tag = vance_get_theme_mod('vance_heritage_prod_tag', 'Our Flagship Product');
        $prod_title = vance_get_theme_mod('vance_heritage_prod_title', 'Introducing EPAVANCE');
        $prod_desc = vance_get_theme_mod('vance_heritage_prod_desc', 'EPAVANCE is a pharma-grade Omega-3 medical food especially formulated for patients with Inflammatory Bowel Disease. Unlike generic supplements, EPAVANCE is developed under the same rigorous manufacturing standards applied to licensed medicines.');
        $prod_btn = vance_get_theme_mod('vance_heritage_prod_btn', 'Learn More About EPAVANCE');
        $prod_url = vance_get_theme_mod('vance_heritage_prod_url', '#');
        $styles = vance_get_section_style_heritage('vance_heritage_product', '#fff');
    ?>
    ...
    <!-- PLATFORM SECTION -->
    <?php if (vance_get_theme_mod('vance_heritage_platform_show', true)) : 
        $plat_tag = vance_get_theme_mod('vance_heritage_plat_tag', 'The Digital Layer');
        $plat_title = vance_get_theme_mod('vance_heritage_plat_title', 'The Vance Medical Platform');
        $plat_desc = vance_get_theme_mod('vance_heritage_plat_desc', 'Beyond our medical food products, Vance Medical is building a world-class digital health hub - combining clinical-grade content, AI-powered tools, and a vibrant community for patients and healthcare professionals.');
        $styles = vance_get_section_style_heritage('vance_heritage_platform', '#142846');
    ?>
    ...
    <?php for ($p=1; $p<=2; $p++) : 
        ...
        if (get_theme_mod($prefix . '_show', true)) :
            ...
            $styles = vance_get_section_style_heritage($prefix, '#fff');
    ...
    <!-- CTA STRIP -->
    <?php if (vance_get_theme_mod('vance_heritage_cta_show', true)) : 
        $cta_title = vance_get_theme_mod('vance_heritage_cta_title', 'Join the Vance Medical Community');
        $cta_desc = vance_get_theme_mod('vance_heritage_cta_desc', "Whether you're a patient managing IBD, a clinician advancing your practice, or a researcher exploring gut health - there's a place for you at Vance Medical.");
        $cta_btn1_lbl = vance_get_theme_mod('vance_heritage_cta_btn1_label', "I'm a Patient");
        $cta_btn1_url = vance_get_theme_mod('vance_heritage_cta_btn1_url', '/patients/');
        $cta_btn2_lbl = vance_get_theme_mod('vance_heritage_cta_btn2_label', "I'm a Healthcare Professional");
        $cta_btn2_url = vance_get_theme_mod('vance_heritage_cta_btn2_url', '/healthcare-professionals/');
        $styles = vance_get_section_style_heritage('vance_heritage_cta', '#EB5A33');
    ?>
