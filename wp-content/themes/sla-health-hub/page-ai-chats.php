...
foreach ( $vac_rows as $row ) {
	...
	foreach ( $chats_raw as $c ) {
		...
		$chats_out[] = array(
			'title'    => ! empty( $c['title'] ) ? $c['title'] : __( 'Untitled conversation', 'sla-health-hub' ),
			'date'     => $c['date'] ?? '',
			'updated'  => $c['updated'] ?? '',
			'messages' => $msgs,
		);
	...
	$vac_users[] = array(
		...
		'name'    => $wp_user ? $wp_user->display_name : ( __( 'User', 'sla-health-hub' ) . ' #' . $row->user_id ),
		'chats'   => $chats_out,
	);
}
...
<div class="page">
	<div class="banner">
		<p class="eyebrow"><?php esc_html_e( 'Vance Medical Hub &middot; Admin only', 'sla-health-hub' ); ?></p>
		<h1><?php esc_html_e( 'VANCE-ai Chat Archive', 'sla-health-hub' ); ?></h1>
		<p class="banner-sub"><?php esc_html_e( 'Every conversation saved to a user’s dashboard, queried live from user meta. Visible to administrators only.', 'sla-health-hub' ); ?></p>
		<div class="stat-row">
			<div class="stat"><div class="stat-num"><?php echo (int) count( $vac_users ); ?></div><div class="stat-label"><?php esc_html_e( 'Users', 'sla-health-hub' ); ?></div></div>
			<div class="stat"><div class="stat-num"><?php echo (int) $vac_total_chats; ?></div><div class="stat-label"><?php esc_html_e( 'Conversations', 'sla-health-hub' ); ?></div></div>
			<div class="stat"><div class="stat-num"><?php echo (int) $vac_total_messages; ?></div><div class="stat-label"><?php esc_html_e( 'Messages', 'sla-health-hub' ); ?></div></div>
			<div class="stat"><div class="stat-num" style="font-size:14px; padding-top:4px;"><?php echo $vac_date_range ? wp_kses( $vac_date_range, array() ) : '&mdash;'; ?></div><div class="stat-label"><?php esc_html_e( 'Date range', 'sla-health-hub' ); ?></div></div>
		</div>
	</div>

	<?php if ( empty( $vac_users ) ) : ?>

		<p class="empty-state"><?php esc_html_e( 'No saved conversations yet.', 'sla-health-hub' ); ?></p>

	<?php else : ?>

		<div class="controls">
			<div class="controls-row">
				<input class="search-box" id="vac-search" type="search" placeholder="<?php esc_attr_e( 'Search titles, messages, users&hellip;', 'sla-health-hub' ); ?>" autocomplete="off">
				<button class="btn" id="vac-expand-all" type="button"><?php esc_html_e( 'Expand all', 'sla-health-hub' ); ?></button>
				<button class="btn" id="vac-collapse-all" type="button"><?php esc_html_e( 'Collapse all', 'sla-health-hub' ); ?></button>
			</div>
			<div class="chip-row">
					...
					<header class="user-head">
						...
						<div class="user-count">
							...
							printf(
								...
								esc_html( _n( '%d conversation', '%d conversations', count( $u['chats'] ), 'sla-health-hub' ) ),
								(int) count( $u['chats'] )
							);
							?>
						</div>
							...
							<article class="chat-card">
								<button class="chat-head" type="button" aria-expanded="false">
									<div class="chat-head-main">
										...
										<div class="chat-meta">
											<span class="meta-item"><?php echo (int) count( $c['messages'] ); ?> <?php esc_html_e( 'messages', 'sla-health-hub' ); ?></span>
											<span class="dot">&middot;</span>
											<span class="meta-item"><?php esc_html_e( 'created', 'sla-health-hub' ); ?> <?php echo esc_html( vance_ai_chats_fmt_date( $c['date'] ) ); ?></span>
											<?php if ( $c['updated'] && $c['updated'] !== $c['date'] ) : ?>
												<span class="dot">&middot;</span>
												<span class="meta-item"><?php esc_html_e( 'updated', 'sla-health-hub' ); ?> <?php echo esc_html( vance_ai_chats_fmt_date( $c['updated'] ) ); ?></span>
											<?php endif; ?>
										</div>
									</div>
								...
								<div class="chat-transcript">
									<?php foreach ( $c['messages'] as $m ) : ?>
										<?php $role_label = $m['label'] ? $m['label'] : ( 'user' === $m['role'] ? __( 'You', 'sla-health-hub' ) : 'VANCE-ai' ); ?>
	...
	<div class="footer-note"><?php esc_html_e( 'Live query against wp_usermeta._sla_saved_chats — refresh to see new conversations.', 'sla-health-hub' ); ?></div>
</div>
